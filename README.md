...
- **关键字搜索**：筛选栏搜索事件（标题 / 标签 / 分类 / 描述加权匹配），匹配项置顶并自动召回到波轮正面，高亮展示；`Enter` 循环跳转、`Esc` 清空
- **工作台模式**：内嵌 Excalidraw 编辑器（本地部署，完全离线），支持多画布、自动保存、PNG / SVG 导出
- **存到事件**：工作台画布一键导出为 PNG 附加到任意事件
- **成果 / 进展时间线**：每个事件可记录多条成果或进展（内容 + 日期），以时间线展示，可增删
- **三栏详情大卡**：点开事件后弹出 左=成果进度 / 中=事件详情 / 右=图片 的三栏布局，栏间拖拽手柄可自由调节宽度（自动记忆）
